...
20. **"capability-control 就是'加个 guardrail'"**——反直觉：它是贯穿 tool schema/上下文策略/运行时权限/身份/可审计/人工批准的**设计轴**，不是安全附加项（Harness 综述的核心论断之一）。更多能力=更大控制问题=更大爆炸半径；每扩一个工具/记忆/对外通道，都要沿这条轴同步扩控制，否则就是 capability abuse 的温床。把它当成上线前"补一个 guardrail"，恰好是治理沦为事后补丁的典型形态。
...
### ▍Agent 工程分层架构与 Harness Engineering
...
| 「harness 是依赖结构而非组件清单」+ 五缺口 + 综述结尾的五个开放问题 | ⭐ | 开放题/研究岗的弹药库 |

---


**⚠️ 易错/反直觉：**
1. **把 harness 当 checklist（"ETCLOVG 七层我都有了"）而非依赖结构。** 综述"依赖结构"论断的靶子就是这种思维：齐备性不等于健壮性，真正的问题是"这层坏了哪些层跟着失效、这层改了哪些层的假设被打破"。面试中报菜名式列举七层只得基础分，能讲出层间失效链才是资深答案。
...
7. **把 O 层理解成"加个日志"。** 一等层意味着 trace 是与执行、工具同级的基础设施（Langfuse+OTel 全链路、token/延迟/错误/成本四类信号关联）；且综述把"从 trace 诊断失败"明确列为仍未解决的开放问题——数据采到 ≠ 能归因。
8. **以为治理可以集中在一处或事后补。** Table 4 的现实是覆盖普遍稀疏、治理沦为事后补丁；但 G 层与业务逻辑、模型行为、组织结构三重耦合，是七层里**最贵的事后补丁**。正确排序：先 ②③ hook 的硬闸门（权限/破坏性动作），再 ①④ 的软审计。
9. **"模型越强越不需要 harness"——方向反了。** 能力↑ ⇒ 控制问题↑（capability–control 轴）：SandboxEscapeBench 显示恰恰是前沿模型最会利用沙箱弱点。harness 不是给弱模型的拐杖，而是给强模型的缰绳；综述列出的另一个开放问题（模型变强时如何让脚手架不变成束缚）说的是另一个方向的难题——拆脚手架的时机，而非不需要缰绳。
...
### ▍应用形态专题：GUI·语音与端侧 Agent

| 考点 | 高频度 | 一句话答法 |
|---|---|---|
| GUI 输入表征三路线（纯视觉 / DOM·AX tree / 混合+SoM） | ⭐⭐⭐ | 通用性 vs 精确性；SoM 把 grounding 从坐标回归降维成编号分类，代价是漏标与动态页面编号漂移 |
| Grounding 为何是第一大失败来源 | ⭐⭐⭐ | 语义意图→坐标/元素映射；实测典型失败是"规划对但点偏"；ScreenSpot-Pro 暴露通用 VLM 在专业软件上的定位 gap |
| OSWorld 为何远低于 SWE-bench | ⭐⭐⭐ | 连续动作空间＋误差乘法累积＋没有编译器/单测式廉价验证器＋部分可观察难重置；本质考"验证器质量决定能力上限" |
| computer-use 主线与浏览器宿主化 | ⭐⭐ | 2024.10 Claude computer-use → 2025.1 Operator/CUA → 2025.7 ChatGPT Agent；UI-TARS 走原生模型路线；2025H2 Atlas/Comet/Claude for Chrome 让浏览器变 Agent 宿主——复用登录态免托管 VM，攻击面同步放大 |
| GUI 安全：lethal trifecta 全凑齐 | ⭐⭐ | 读网页=不可信内容、登录态=私有数据、操作浏览器=对外行动；缓解四件套：隔离实例/凭证保险箱/终态人工确认/域白名单（机制见第 9 章） |
| 级联 vs 端到端语音路线选型 | ⭐⭐⭐ | 级联有文本中间态可 guardrail/审计/复用工具生态但延迟叠加且丢副语言；端到端低延迟保语气但可控性弱；B 端客服级联、C 端陪伴端到端——按场景选，不站队 |
| voice-to-voice 延迟预算与流式三级并行 | ⭐⭐⭐ | 四段预算对标人类 200–300ms 轮转期望；partial ASR→LLM 投机推理→TTS 分句流水线，把串行加法变"关键路径最长者"；工具延迟用垫话藏 |
| 判停（turn detection）与 barge-in 三连 | ⭐⭐⭐ | "说完了吗"是语义问题不是音量问题——语义判停动态调静音阈值；打断三连：停播冲缓冲＋取消生成＋历史截断到实听位置；backchannel 不触发打断 |
| τ³-bench 语音能力洼地 | ⭐⭐ | 同任务语音成绩约为文本 30–45%（第 8 章）；"上语音会显著变笨"的定量弹药，也是客服选级联的论据 |
| 端侧三重红利 vs 能力差距约束 | ⭐⭐⭐ | 隐私（唯一云端不可替代）/延迟与离线可用/边际成本趋零；与云端旗舰约两个数量级参数差距→核心是重新划分任务边界，不是把云端搬下来 |
| 端侧 decode 内存带宽墙 | ⭐⭐⭐ | decode 是 memory-bound：每 token 通读全部权重；NPU TOPS 主要救 compute-bound 的 prefill；4-bit 量化双重收益（省内存＋等比减读带宽） |
| 端云路由与 Apple PCC | ⭐⭐ | 与第 10 章模型级联同构，多了隐私/网络/电量路由维度；PCC（无状态、可审计）证明升云不等于弃隐私；也是 NVIDIA SLM 立场文（第 11 章）的物理实现 |
| 手机权限通道 | ⭐⭐ | Android Accessibility（读控件树+代发事件）vs Apple App Intents（声明式语义调用）vs 纯视觉；权限与生态是比算力更硬的约束 |
...
- **人类轮转期望 200–300ms**，沉默超 1 秒触发"喂？还在吗"；级联 voice-to-voice **1 秒内算优秀、1.5 秒以上明显卡顿**（量级参考）。
- 延迟四段预算：**VAD 判停 200–700ms**（常为第一大头，且是"故意等出来的"）/ ASR 尾包 100–300ms / LLM TTFT 200–800ms / TTS 首块 100–300ms。
- **OSWorld**：真实 Ubuntu 桌面、369 个跨应用任务、程序化终态验收脚本；**OSWorld-Verified 前沿约 60% vs 人类约 72%**。
- 误差乘法累积：**30 步任务单步 97% 正确，整体成功率只剩约四成**。
- **ScreenSpot-Pro**：初代通用多模态模型得分**普遍不足 20%**、部分接近个位数——GUI 专用 grounding 预训练（OS-Atlas/ShowUI/UI-TARS）的动机。
- **τ³-bench**：同任务语音成绩约为文本 **30–45%**。
- 端侧：**3B 模型 4-bit 约 1.5–2GB**；手机 LPDDR **数十 GB/s** vs 数据中心 GPU 数 TB/s（差约百倍）；**生成速度上限 ≈ 内存带宽 ÷ 权重字节数**，与 TOPS 无关。
- 电话：**PSTN 是 8kHz 窄带（G.711）**，ASR 准确率显著低于 16kHz 宽带；接入走 Twilio Media Streams。

---


**⚠️ 易错/反直觉：**
1. **把"DOM 方案省 token"当优点** — 真实页面 DOM 动辄数万 token，常比一张截图更贵；正确卖点是定位精确、可程序化校验，代价是必须激进裁剪且裁剪丢信息。
2. **认为 SoM 已彻底解决 grounding** — 它只覆盖结构可枚举元素，Canvas/自绘控件/桌面应用仍需回退坐标回归；标注管线的漏标率就是方案上限，动态刷新还有编号漂移（需每步重标注）。
3. **把 OSWorld"前沿约 60% vs 人类约 72%"读成接近人类** — 基准是有限任务集+验收脚本，真实开放环境的长尾（弹窗、界面改版、验证码、A/B 实验）不在分布内；数字对比仅在基准内部有效。
4. **以为语音延迟大头在 LLM** — 实际 VAD 静音等待往往是第一大头，砍它靠语义判停动态缩短等待而非换更快的模型；只优化 TTFT 会系统性押错重点。
5. **barge-in 只停播放、不回滚历史** — 对话历史里留着用户根本没听到的半段话，后续指代与承诺全部错位；必须按已播放时长把 assistant 消息截断到实听位置（OpenAI Realtime API 为此提供 truncate 语义）。
6. **把端到端 speech-to-speech 当无脑升级** — 自然度与延迟占优，但没有文本中间态可拦截、工具调用与 policy following 显著弱于文本路线（τ³-bench 的 30–45% 即证据）；客服类强规则场景直接翻车。
7. **把端侧瓶颈归因"算力不够"** — decode 是 memory-bound、每 token 要通读全部权重，天花板是内存带宽；答"换更强 NPU 就能提速生成"暴露对推理两阶段特征的不理解，优化方向是减少每 token 读的字节数（更低比特量化、KV 量化、投机解码）。
8. **只算权重、不算 KV cache 的内存账** — 手机 RAM 8–16GB 且无 swap 余地，由系统、前台 App、权重、KV cache 共享；长上下文很快让"模型装得下"变成"对话跑不动"，必须按"权重+KV+系统余量"整体核算。
9. **把"升云"等同"放弃隐私"** — Apple PCC 用无状态计算+可审计镜像给出可验证的隐私保证；隐私应作为路由的连续维度而非端/云二分（反过来也不能把 PCC 说成普通云 API）。
10. **GUI 选型题只答单路线** — DOM 或纯视觉二选一都是不完整答案；标准答案是混合分层：AX tree 枚举+SoM 标注做感知，动作三级回退（元素句柄→SoM 编号→原始坐标），验证层独立双通道（DOM/URL 断言+截图复核，矛盾时以渲染结果为准）。
