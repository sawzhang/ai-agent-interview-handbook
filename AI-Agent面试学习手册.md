...
│   └── 评估驱动开发（EDD）：目标与门禁前置 → 错误分析先于指标 → evaluator 小而锐 → 一次只动一个变量
├── 2. 评估维度
...
│   └── 联合回归 vs 组件隔离评估
└── 11. 评估的组织与运营
    ├── 角色分工：rubric 与 golden set 归产品团队，工具与埋点归平台团队
    ├── 人类先对齐再自动化：专家标注分歧未解决前不调 judge；设最终裁决人
    ├── 规模阶梯：~100 条人读 → 50–100 条校准集 → 数千条扩量 → 线上按比例抽样
    └── 运营节奏：周度评审、judge 定期重校准、失败类目趋势复盘
```
...
#### 2.15 Eval-Driven Development：把评估前移为开发范式

前面十四节讲的是"评估这套系统本身怎么建"；这一节讲**它怎么嵌进开发流程与团队**。Airbnb 把这套做法命名为 **eval-driven development（EDD）**：像 TDD 那样让评估先于实现存在，用它持续发现、编码、回归失败模式。"评估这件事谁做、怎么排期"这类组织题，标准答法就在这里。

**（1）EDD 与 TDD 的三处断裂**（能说出断裂点比会背类比值钱得多）：

| | TDD | EDD |
|---|---|---|
| 断言 | 二值、确定，写完即可信 | judge 是有偏分类器，**尺子本身要先被校准**（见 2.4） |
| "变绿"时刻 | 存在：红 → 绿 | 不存在：pass rate 是分布，只有**阈值与门禁** |
| 用例来源 | 需求规格 | 主体来自**线上真实失败**的持续挖掘（见 2.8） |

由此推出两条硬规矩：① **目标与门禁必须在开工前定死**（测哪个指标、阈值多少、谁有权放行）——事后定阈值等于按结果凑标准；② 迭代时**一次只动一个变量**（换模型、改 prompt、改检索不要一起上），否则分数变化无法归因。

一个常被忽略的副产品：**写 rubric 的过程逼产品方把"好"外部化**。"这个回答好不好"在 rubric 落地之前其实没有定义——这也是 rubric 必须由领域专家/PM 主笔、而非工程师独写的根本原因。

**（2）错误分析先于指标（bottom-up），与 2.12 互补而非替代**

Airbnb 的起手式是"拿不准就去看数据"：先跑约 100 条真实输入，**逐条人工读输出与 trace**，把错误归类，再由观察到的类目决定建哪些 evaluator——而不是先想一套通用指标。他们给的客服 bot 示例分布是：编造源文档里没有的政策细节 15 条、正确但过于啰嗦 8 条、该答却拒答 5 条、JSON 格式坏 3 条。**这是他们的分布，不是普适分布**——自己的产品要自己数。

这一步专治 **false confidence**：通用 "helpfulness" 分很容易做得漂亮，却与产品真实失败模式无关，上线后指标全绿、投诉照旧。

与 2.12 的关系是互补：2.12 是 top-down（任务分类学 → 能力×难度覆盖矩阵 → item spec），保证构念效度、**防漏测**；错误分析是 bottom-up，保证指标对齐真实失败、**防错测**。冷启动时 bottom-up 先行——覆盖矩阵的"格子"标签本身就该来自真实错误类目，凭空想出的分类学往往漏掉最贵的那一类。

面试话术差别很大：被问"你怎么决定评估哪些维度"，答"我读了 N 条线上输出做错误分类，前三类占了 X%，evaluator 就先建这三个"，远胜于背一串通用指标名。

**（3）evaluator 集要小而锐**

3–5 个校准良好的 judge 优于 20–30 个噪声 judge。配套两条：**一个维度一个 evaluator**，不要造"全能 judge"（呼应 2.4 的分维度打分）；rubric 里**歧义是敌人**——判据如果人都无法一致执行，模型更不可能。指标膨胀的代价不只是成本：越多越没人看、越容易互相矛盾，也没人有精力维持每把尺子的校准。

**（4）人类先对齐，再谈自动化**

最锋利的一条：**两位领域专家对同一条样本的标签都不一致时，停下——不要去调 judge prompt**。此刻没被定义的是"好"本身，该做的是改 rubric 或让产品拍板。2.8 说过 IAA "同时是 rubric 质量的度量"，这里把它推成一条硬停止规则：**人都没对齐的东西，自动化只会把分歧固化成噪声**。配套是**指定最终裁决人**（通常是 PM），否则 rubric 永远收敛不了。

**（5）规模阶梯与运营节奏**（给量级比给原则更能证明做过）

```
~100 条人工通读 → 错误分类表
  → 50–100 条校准集（必须含坏例，否则测不出 judge 的辨别力），领域专家标注
  → 校准循环：量化 judge-人一致性 → 分析分歧 → 改 rubric / few-shot → 重跑
  → 达标后扩量到数千条
  → 同一套 evaluator 镜像到线上：按比例抽样（Airbnb 用 5% 日流量，脱敏后人审）
  → 固定周度评审：新失败模式 → 新 evaluator，回到第一步
```

**一致性口径的坑**：Airbnb 报的达标线是 judge-人**一致率**（高 80%–90%），本章 2.4 报的是 Cohen's κ——两者不可互换。二值合规判断这类正例稀少的场景正是患病率悖论的高发区，会出现"一致率 90% 但 κ 很低"。**引用任何一致性数字前先问口径**，稳妥做法是一致率 + κ + ROC-AUC 一起报。

**（6）平台团队 × 产品团队的分工**

中心基础设施团队提供共享工具与最佳实践（trace 埋点、judge 编排、一致性计算、回归调度）；**rubric 与 golden set 归产品团队**——"好"是领域知识，不能中心化，也不该有一套放之四海的评分标准。这条分工线就是"评估如何在组织里规模化"的标准答案。
...
| 错误分析先于指标 | ⭐⭐ | 先读 ~100 条做错误分类，再由类目决定建哪几个 evaluator |
| Eval-Driven Development 与 TDD 的差异 | ⭐⭐ | 断言要先校准、没有"变绿"时刻、用例从线上挖；门禁前置 |
...
| 评估的组织分工与运营节奏 | ⭐ | evaluator 小而锐、专家分歧先解决、最终裁决人、平台/产品分工 |
...
#### 题 14（开放/组织）：一个新的 GenAI 功能要建评估，第一周你做什么？这件事在团队里谁做？
...
22. **先定指标再看数据**：通用 helpfulness / 一致性分很容易做得漂亮，却与产品真实失败模式无关——上线后指标全绿、投诉照旧。顺序必须反过来：先人工读一批输出做错误分类，再由类目决定建哪些 evaluator。
23. **以为 evaluator 越多越安全**：20–30 个未校准的指标不如 3–5 个校准过的。指标膨胀稀释注意力、制造互相矛盾的信号，且没人有精力维持每把尺子的校准。
24. **把标注分歧当标注员的问题**：两位领域专家对同一条样本标不一致时，真正没定义的是"好"本身。此时该改 rubric / 让产品拍板，而不是去调 judge prompt——人都没对齐的东西，自动化只会把分歧固化成噪声。
